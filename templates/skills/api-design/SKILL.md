---
name: api-design
description: <One line naming the policy and when it applies. Example: Apply the organization's API design policy to any change that touches <area>. Use during Design, Build, and Review.>
metadata:
  owner: "Platform lead"
...
API design policy without a human having to remember the rules.

## Who owns this

**Platform lead.** Changes to this file require their approval. Record the date of the
...
4. If a rule is ambiguous for this change, do not guess: flag it in the spec's
   **Areas of concern** table and route it to Platform lead.

## Output

- A per-rule verdict with citations.
- Findings routed to the review artifact.
- Anything ambiguous routed to Platform lead rather than resolved by the agent.
